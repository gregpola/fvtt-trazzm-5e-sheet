...
* adapted for the TRAZZM5ESheet from
...
export const addFavorites = async function(app, html, data, position) {
  ...
  for (let item of items) {
      ...
      if (app.options.editable) {
        let favBtn = $(`<a class="item-control item-fav" data-fav="${isFav}" title="${isFav ? game.i18n.localize("TRAZZM5E.RemoveFav") : game.i18n.localize("TRAZZM5E.AddFav")}"><i class="${isFav ? "fas fa-bookmark" : "far fa-bookmark"}"></i></a>`);
        favBtn.click(ev => {
      ...
      await loadTemplates(['modules/trazzm-5e-sheet/templates/favorites/item.hbs']);
      let favHtml = $(await renderTemplate('modules/trazzm-5e-sheet/templates/favorites/template.hbs', data));
    ...
    Hooks.callAll("renderedTrazzm5eSheet", app, html, data);
  }